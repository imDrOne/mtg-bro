...
class CardLimitedStatsCollectionServiceTest {
    ...
    fun `collectAll sends start and finish alerts when all match types succeed`() = runBlocking {
        whenever(lands17ApiClientFacade.getDraftStatistic(eq("BLB")))
            .thenReturn(listOf(response("Draft Card", 1)))
        whenever(lands17ApiClientFacade.getSealedStatistic(eq("BLB")))
            .thenReturn(listOf(response("Sealed Card", 2)))
        val service = service()
        ...
        then(cardLimitedStatsRepository).should(times(2)).saveAll(any())
    }

    @Test
    fun `collectAll sends parsing alert and still finishes when one match type fails to parse`() = runBlocking {
        whenever(lands17ApiClientFacade.getDraftStatistic(eq("BLB")))
            .thenThrow(RuntimeException("17lands unavailable"))
        whenever(lands17ApiClientFacade.getSealedStatistic(eq("BLB")))
            .thenReturn(listOf(response("Sealed Card", 2)))
        val service = service()
    ...
    fun `collectAll sends db alert when saving fails`() = runBlocking {
        ...
        willThrow(RuntimeException("db down"))
            .given(cardLimitedStatsRepository)
            .saveAll(any())
        val service = service()

        service.collectAll("BLB")

        then(alertService).should(times(2)).savingFailed(eq("BLB"), any(), eq(1), any())
